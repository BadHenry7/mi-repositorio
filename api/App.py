...
def rutina():
    try:
        ...
        cur = mysql.connection.cursor() #conectar con la base de datos ↓
        cur.execute('SELECT tipo_rutina FROM rutina where tipo_rutina =%s',(tipo_rutina,)) #ejecutar el scrip
        rv = cur.fetchall() #consultar todos los registros
        cur.close() # cierra la conexion que abrimos arriba        ↑
        payload = []    #lista o array, arreglo, como quieran decirle
        content = {}    #estructura de tipo objeto
...
def BuscarEntrenador():
    try:
        ...
        for result in rv:
            content = {'id': result[0], 'nombre':result[1],  'direccion':result[3], 'correo':result[4] , 'Especialidad': result[10], 'estado':result[8] }
            #id = result[0]   
            payload.append(content)
            content = {}
      ...
      #  vusuario= request.json['id_usuario']
       ...
       # fecha_registro = datetime.now()
        ...
        cur.execute("""
                    ...
                    GROUP BY HOUR(fecha) 
                    ORDER BY hora;
""")  #EY MANO VEN ACA PORQUE ARRIBA HAY TANAS VAINAS EN VERDE
...
def estadisticas_por_mes():
    try:
        ...
        cur.execute("""SELECT DAYNAME(fecha) as dia_semana, COUNT(*) as total_asistencia 
            ...
            WHERE YEAR(fecha)=2023 AND MONTH(fecha)=8
            GROUP BY
            DAYNAME(fecha) ORDER BY (fecha)""")  #SET lc_time_names = 'es_ES';
...
"""
SELECT e.name, u.entrenador_entrenar FROM usuario as u, entrenador as e 
